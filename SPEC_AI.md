...
## Self-host
Host is a stack VM. Compiler: `std/{lexer,parser,compile,compiler}.nl` as `std/*.nlc`. VM in NL: `std/vm.nl`. Native kernel: `rt/*.c` (`gcc rt/*.c` — no Go). `evaluator.nl` is a debug tree-walk.
`C.nl_eval(code, env)` = parse + compile + `vm_run`. `env=null` => `copy(builtins())`.
Last map in a module file is the export; otherwise all top-level names. After boot, `use`/`load` `vm_run` a sibling `.nlc` when it is newer than the source, otherwise compile with `C.nl_parse`/`nl_compile` and write `.nlc`. Boot `vm_run`s committed `std/*.nlc` (`compiler.nlc` is the full module; its `use` loads sibling `.nlc` before `C` exists). Stale chunks are rebuilt by the guest compiler; missing stage-0 `.nlc` is a boot error. Language tests: `tests/*.nl`. `neurolang check` runs them. New syntax desugars in `std/compile.nl`. Native: `gcc -O2 -o neurolang rt/nl.c rt/main.c`.
