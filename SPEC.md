...
`neurolang run` / `eval` / `repl` boot **`std/compiler`** then `C.nl_eval` on the VM. The Go lexer/parser remain only to load `std/compiler` when bytecode cache is missing or stale. `std/{lexer,parser,compile}.nlc` are guest-compiled chunks; a fresh cache boots with `vm_run` plus `compiler.nl` glue (the `use` lines stripped). After that, `use`/`load` compile modules with `C.nl_eval`. `neurolang self` is an alias of `run`. `!ident` is a tool call; write boolean not as `!(expr)` or `x == false`.
